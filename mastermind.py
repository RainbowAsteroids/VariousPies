...
#I connect 220 ohm resistors to LEDS
...
buttonD = 10 #Pin for the debug button
...
def resetKey(returnKey=False): #Resets the randomized key
		global passkey
		key = [randomInt(1,5), randomInt(1,5), randomInt(1,5), randomInt(1,5)]
		if returnKey:
				return key
		else:
				passkey = key
		...
		while True:
				...
				if inputtedKey == passkey: #Runs the victory code
						...
						while loop:
								...
								elif GPIO.input(button4): #Makes a new light show and plays that
									telegraph(resetKey(True))
